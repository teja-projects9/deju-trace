...
(function boot() {
  ...
  /**
   * The Tree and Files per-line time cell: percent leads, since it is the figure that is
   * comparable line to line, with the duration bracketed alongside it — "12% [8.4ms]".
   * Square brackets rather than round: parentheses already carry a softer "aside" meaning
   * elsewhere in this UI (the branch-coverage count beside a line), and this is not an
   * aside. Falls back to the bare duration when there is no run length to divide by.
   */
  function fmtLineTime(micros) {
    var p = pct(micros);
    return p ? p + ' [' + fmt(micros) + ']' : fmt(micros);
  }
  ...
  function lineRow(rec, f) {
    ...
    if (micros != null) {
      tm.textContent = (l.methodTotalMicros != null ? '▸ ' : '') + fmtLineTime(micros);
      ...
      var lp = pct(micros);
      tm.title = (l.methodTotalMicros != null ? 'method total, summed across every call in the run'
          : 'line self time, summed across every call to this method in the run')
        + (lp ? ' · ' + lp + ' of the run' : '');
    }
    var code = el('td', 'code');
  ...
  function sqlRow(node, depth, parentRow) {
    ...
    more.addEventListener('click', function () {
      ...
      // The window scrolling next is what moves this row: paintTreeWindow() clears and
      // rebuilds the table to shift which rows are mounted, and doing that while this
      // button still has focus is what threw the page to wherever the row ended up —
      // the browser's own focus handling, not anything this script asked for. Nothing
      // about the click needs focus to stay here once it has been handled.
      more.blur();
      // This row's height just changed outside the virtual scroller's own measurement
      ...
      if (hh && r.vi != null && rowHeights[r.vi] !== hh) {
        rowHeights[r.vi] = hh;
        rebuildOffsets();
        if (padTop && renderedFrom >= 0) {
          padTop.firstChild.style.height = rowOffsets[renderedFrom] + 'px';
          padBot.firstChild.style.height =
            Math.max(0, rowOffsets[visRows.length] - rowOffsets[renderedTo]) + 'px';
        }
      }
    });
    td.appendChild(more);
  ...
  function frameRow(node, depth, parentRow) {
    ...
    // The source lines shown under this frame are not scoped to this one call — see the
    // note on their own timing cell — so this total needs to say the same thing about
    // itself: it is only this call's own inclusive time, and the lines below may add up to
    // a bigger number if this method was called more than once elsewhere in the run.
    time.title = "total (inclusive) time of this one call. The lines below total every call "
      + 'to this method in the run, which can be larger.';
  ...
  function codeRow(lineModel, file, depth, parentRow) {
    ...
    if (lineModel.timeMicros != null) {
      tm.textContent = fmtLineTime(lineModel.timeMicros);
      ...
      // Summed across every call to this method anywhere in the run, not just the one
      // step this row happens to be nested under — the source model has no per-invocation
      // breakdown to draw on, only one running total per line. Left unsaid, a line under a
      // frame that reads "3.6 ms" but shows "442 ms" of its own looks like a bug rather
      // than what it is: 400 calls' worth of time landing on the one line all of them share.
      tm.title = 'line self time, summed across every call to this method in the run'
        + (lp ? ' · ' + lp + ' of the run' : '');
    }
  ...
  byId('fidelity').textContent = treeAvailable
    ? 'Steps are in exact execution order. Lines within a method are in source order and '
      ...
      + '— which also means a line\'s own time total is summed across every call to that '
      + 'method, not just the one step it happens to be shown under, and can run higher than '
  ...
  var findings = [];
  // Collapsed by severity, not remembered across a reload: a long "for information" group
  // is the one worth folding away on open, without it staying folded the next time the
  // report is opened and hiding a finding that matters on a different run.
  var findingsCollapsed = { info: true };
  ...
  function addFinding(sev, title, detail, seq, sql) {
  ...
  function findSlowQueries(runMicros) {
    ...
    addFinding(share >= 0.3 ? 'high' : 'med',
      ...
      worst.seq, shorten(String(worst.sql).replace(/\s+/g, ' ').trim(), 240));
  }
  ...
  function renderFindings() {
    ...
    var shown = fq === '' ? findings : findings.filter(function (f) {
      return (f.title + ' ' + f.detail + ' ' + (f.sql || '')).toLowerCase().indexOf(fq) !== -1;
    });
    ...
    var frag = document.createDocumentFragment();
    FINDING_SEVS.forEach(function (grp) {
      var items = shown.filter(function (f) { return f.sev === grp.sev; });
      if (!items.length) return;
      frag.appendChild(buildFindingGroup(grp, items));
    });
  ...
  /** Severity groups in the order the list already sorts by: worst first. */
  var FINDING_SEVS = [
    { sev: 'high', label: 'Look here' },
    { sev: 'med', label: 'Worth a look' },
    { sev: 'info', label: 'For information' }
  ];

  /**
   * One severity's section: a header naming it and how many findings it holds, and its
   * cards underneath. Collapsible per severity, so a long "for information" tail can fold
   * away without taking the findings that actually need attention with it.
   */
  function buildFindingGroup(grp, items) {
    var collapsed = !!findingsCollapsed[grp.sev];
    var section = el('div', 'findgroup ' + grp.sev);
    var head = el('button', 'findgroup-head');
    head.type = 'button';
    head.setAttribute('aria-expanded', String(!collapsed));
    head.appendChild(el('span', 'findgroup-chevron', collapsed ? '▸' : '▾'));
    head.appendChild(el('span', 'findgroup-label', grp.label));
    head.appendChild(el('span', 'findgroup-count', String(items.length)));
    head.addEventListener('click', function () {
      findingsCollapsed[grp.sev] = !collapsed;
      renderFindings();
    });
    section.appendChild(head);

    var body = el('div', 'findgroup-body');
    body.hidden = collapsed;
    items.forEach(function (f) { body.appendChild(buildFindingCard(f)); });
  ...
  function buildFindingCard(f) {
    ...
    card.appendChild(p);
    if (f.sql) {
      // Same tokenized, coloured rendering the Call Tree and Flame tooltip give a query,
      // so a statement reads the same way wherever this report shows one.
      var code = el('pre', 'sqltext');
      paintSql(code, f.sql);
  ...
  // Narrowest a flame frame may be before it also carries time + share text, on top of its
  // name. Below this, width already says how long the call took the way a classic flame
  // graph reads; above it, there is room enough that leaving the numbers to the tooltip
  // only would make a reader hover every box just to compare two of them.
  var FLAME_METRICS_MIN_W = 90;
  ...
  var flameMergeBtn = byId('flameMergeBtn');
  var flameCallersBtn = byId('flameCallersBtn');
  var flowStepsBtn = byId('flowStepsBtn');
  var flowFlameBtn = byId('flowFlameBtn');
  var flowChartBtn = byId('flowChartBtn');
  var chartOrientBtn = byId('chartOrientBtn');
  var chartLayerBtn = byId('chartLayerBtn');
  var chartMinimapBtn = byId('chartMinimapBtn');
  var chartLegend = byId('chartLegend');
  ...
  var flowZoomSlider = byId('flowZoomSlider');
  var flowZoomValue = byId('flowZoomValue');
  ...
  var flowMinimap = null;      // small overview inset, pinned over a corner of the viewport
  ...
  var flowMatchSelf = null;    // index -> 1 when the pattern filter's own text matched this step
  var flowMatchTotal = 0;      // summed micros of every outermost matched subtree
  var flameRoot = null;        // seq the flame view is zoomed into, null for the whole run
  /* Merged is what a flame graph normally means, so it is the default; the chronological
     layout stays one click away for the times the question is "in what order", and the
     two are otherwise the same view of the same run. */
  var flameMerged = true;
  var flameRootPath = null;    // call path the merged flame is zoomed into
  var flameMergedTotal = 0;    // micros the widest row represents, for the tooltip's share
  var flameCallersArm = false; // true between clicking "Callers of…" and picking a frame
  var flameCallersTarget = null; // method/SQL key the flame is showing merged callers of, or null
  ...
  var flowChart = true;        // the default layout for the Flow tab
  var chartNodes = [];      // deduped: {key, cls, mth, isSql, sql, status, x,y,w,h, n, total, order, depth}
  var chartByKey = {};
  var chartEdges = [];      // {from, to, x1,y1,x2,y2}
  var chartEdgeByKey = {};  // "fromKey>toKey" -> edge, so the marker can walk the same line it draws
  var chartGroups = [];     // {label, hue, x0,y0,x1,y1, pill} — adjacent same-package siblings
  var chartPath = [];       // {node, reps} per distinct visit, in execution order (sampled if huge);
                             // a call site hit several times in a row (a tight loop) collapses into
                             // one entry so Play doesn't idle through every repeat
  var chartPathSampled = false; // true only when CHART_MAX_PATH truncation actually kicked in
  var chartLastKey = null;  // key of whichever call actually ran last, the only one that leads to End
  var chartContentW = 0;
  var chartContentH = 0;
  ...
  // Tall enough for three stacked lines (class, method, time/share/count) instead of the one
  // line the box used to hold — a call site's identity no longer has to fight its own metrics
  // for the same 34px of width.
  var CHART_NODE_H = 58;
  ...
  var CHART_DOT_R = 6;
  // Start/End draw as a pill narrower than the grid cell they're centred in (see
  // drawChartTerminator); an edge that stops at the cell's own edge instead of the pill's
  // actual visible edge leaves a gap wherever the two don't line up.
  var CHART_TERM_W = 108;
  var chartVertical = true;  // false: depth -> columns (left to right). true: depth -> rows (top to bottom).
  var chartSortByLayer = false;  // false: siblings in call order. true: grouped by guessed architectural layer.
  var chartMinimapOn = false;  // an overview corner inset, for finding your place in a large chart

  /**
   * Which architectural layer a class name suggests, from its own name alone — the report
   * has no build-time knowledge of a project's actual layering, so this is a guess from the
   * naming convention most Java (and similar) codebases already follow, not a real
   * classification. Ranked in the order a request typically flows through them, so sorting
   * siblings by this number reads top-to-bottom (or left-to-right) the way an architecture
   * diagram would group them, rather than the order the trace happened to call them in.
   * Unrecognised names sit between service and repository — the middle of the pipeline is a
   * safer default than either end for a class this guess has nothing to go on.
   */
  function chartLayerRank(n) {
    if (n.isSql) return 2;
    var cls = n.cls || '';
    if (/(controller|resource|endpoint|handler)$/i.test(cls)) return 0;
    if (/(service|manager|usecase|facade)$/i.test(cls)) return 1;
    if (/(repository|dao|mapper|store)$/i.test(cls)) return 2;
    if (/client$/i.test(cls)) return 3;
    return 1.5;
  }

  /** Call order normally; grouped by {@link chartLayerRank} when that sort is switched on,
   *  with call order still breaking ties within the same layer so the grouping is stable. */
  function chartSiblingCompare(a, b) {
    if (chartSortByLayer) {
      var d = chartLayerRank(a) - chartLayerRank(b);
      if (d) return d;
    }
  ...
  function buildFlow() {
    ...
    // Also a child of the spacer, not the wrap, so it shares the canvas's own trick of
    // being manually re-pinned over the scrolled viewport on every scroll event (see
    // paintFlow) rather than actually scrolling away with the diagram underneath it.
    flowMinimap = document.createElement('canvas');
    flowMinimap.id = 'flowMinimap';
    flowSpacer.appendChild(flowMinimap);
    ...
    flowMinimap.addEventListener('mousedown', function (e) {
      minimapDragging = true;
      jumpToMinimapPoint(e);
      e.preventDefault();   // otherwise a drag also selects the canvas as text
    });
    window.addEventListener('mousemove', function (e) {
      if (minimapDragging) jumpToMinimapPoint(e);
    });
  ...
  function updateFlowNote() {
    ...
    // Merged flame counts differently on purpose: nothing there is hidden, so "N of M
    // shown" would be the wrong shape of sentence. What it did is fold, and by how much is
    // the number worth printing.
    var bits = [];
    if (flowFlame && (flameMerged || flameCallersTarget)) {
      var frames = flowDrawn.length;
      bits.push(flowNodes.length + ' calls merged into ' + frames
        + (frames === 1 ? ' frame' : ' frames'));
    } else {
      bits.push(shown === flowNodes.length
    ...
    if (flowFilterText.trim() && flowFlame) {
      // Flame highlights a search rather than hiding around it, the way Brendan Gregg's own
      // flame graph search does: everything stays on screen, matches turn magenta, and the
      // number that matters is how much of the run they cover, not how many steps matched.
      var pct = flowRootTotal > 0 ? (flowMatchTotal / flowRootTotal) * 100 : 0;
      bits.push((flowFilterLiteral ? 'text' : 'pattern') + ' "' + flowFilterText.trim()
        + '" highlighted, ' + (pct >= 0.1 ? pct.toFixed(1) : '<0.1') + '% of total time');
    } else if (flowFilterText.trim()) {
    ...
    flowNote.textContent = bits.join(' · ') + '. '
      + (flowFlame && flameCallersTarget
        ? 'Showing every caller of the frame at the bottom, merged by identity. Click a'
          + ' caller to re-centre on it, or Callers of… in the toolbar to leave.'
  ...
  function computeFlowFilter() {
    ...
    flowMatchSelf = self;
    // Flame's search highlights rather than hides, and the "% of time matched" badge has to
    // avoid double-counting a match nested inside another match — so this walks forward
    // (parents always come first) tracking whether the nearest matched ancestor is the node
    // itself, and only sums a subtree's whole duration at the outermost match in its chain.
    var nearestSelf = new Int8Array(n);
    for (var m = 0; m < n; m++) {
      var pIdx = flowParentIdx[m];
      var ancestorMatched = pIdx >= 0 && nearestSelf[pIdx];
      nearestSelf[m] = self[m] || ancestorMatched ? 1 : 0;
      if (self[m] && !ancestorMatched) flowMatchTotal += flowNodes[m].total;
    }
  ...
  function computeFlowVisible() {
    ...
    while (i < flowNodes.length) {
      ...
      // from every match — so its whole subtree goes with it. Flame is the exception: a
      // search there highlights matches in place rather than hiding the rest, the way
      // Brendan Gregg's own flame graph search behaves, so it never drops a step for this
      // reason.
      if (flowKeep && !flowFlame && !flowKeep[flowIdxBySeq[n.seq]]) { i += n.span; continue; }
  ...
  // ------------------------------------------------------------ merged flame ---
  //
  // What a flame graph is, as against what the chronological layout above draws.
  //
  // The layout above gives every recorded call a box of its own and positions it on a
  // time axis. On a loop that is thousands of boxes, nearly all of them narrower than a
  // pixel, and the width floor that rescues them from invisibility is also what forces
  // the note to admit the widths have stopped meaning anything.
  //
  // A flame graph merges instead: every call that arrived by the same route through the
  // code becomes ONE frame, as wide as the time all of them took together. The count
  // moves into the tooltip. A four-hundred-iteration loop is one readable box rather than
  // four hundred slivers, which is the whole reason the format is read the way it is —
  // and it is what Glowroot, async-profiler and Brendan Gregg's original all do. What is
  // given up is when each call happened; the Timeline tab answers that question already.

  /** Frames narrower than this are dropped with their subtree: too thin to see or hit. */
  var FLAME_MIN_PX = 0.4;

  /**
   * Aggregates every recorded call into frames keyed by call path.
   *
   * <p>Walks {@link flowNodes} in pre-order applying the same filters the chronological
   * view applies — the file picker, excluded types, the SQL toggle — but not its
   * adjacent-run folding, which merging supersedes: adjacency can only fold a run of
   * identical neighbours, and a loop body calling three different things in turn never
   * produces one.
   *
   * <p>Skips are whole subtrees, so what survives is still a tree and a kept node's parent
   * is always the nearest kept node one level up.
   *
   * @return {{roots: Array, total: number}}
   */
  function buildFlameMerged() {
    var byDepth = [];        // depth -> the merged frame the next child belongs under
    var roots = [];
    var rootByKey = {};
    var total = 0;
    var searching = flowFilterText.trim() !== '' && flowMatchSelf;

    for (var i = 0; i < flowNodes.length; i++) {
      var n = flowNodes[i];
      if (n.cname && !classSelected(n.cname)) { i += n.span - 1; continue; }
      if (!showExcluded && foldableSeq[n.seq]) { i += n.span - 1; continue; }
      if (!flowSql && n.isSql) { i += n.span - 1; continue; }

      var parent = n.depth > 0 ? byDepth[n.depth - 1] : null;
      var siblings = parent ? parent.kids : roots;
      var map = parent ? parent.kidByKey : rootByKey;
      var frame = map[n.key];
      if (!frame) {
        frame = map[n.key] = {
          // Everything drawNode, the hit test and the tooltip read off an ordinary flame
          // node, so a merged frame duck-types one and the whole draw path is reused.
          seq: n.seq, cls: n.cls, mth: n.mth, sig: n.sig, isSql: n.isSql, sql: n.sql,
          owner: n.owner, cname: n.cname, key: n.key, sqlLines: n.sqlLines,
          depth: parent ? parent.depth + 1 : 0,
          shown: 0, calls: 0, matched: false,
          kids: [], kidByKey: {}, parent: parent,
          path: (parent ? parent.path + '\u0001' : '') + n.key
        };
        siblings.push(frame);
      }
      frame.shown += n.total;
      frame.calls++;
      if (searching && flowMatchSelf[flowIdxBySeq[n.seq]]) frame.matched = true;
      if (!parent) total += n.total;
      byDepth[n.depth] = frame;
    }
    return { roots: roots, total: total };
  }

  /**
   * Places merged frames: width is time, children packed inside their parent, widest first.
   *
   * <p>Widest first rather than in call order or alphabetically: the question this view is
   * opened with is "what took the time", and putting the answer at the left edge of every
   * row means reading down the left-hand column answers it without hunting.
   *
   * <p>A frame's children can only be as wide as the frame itself, so the parent's own
   * uncovered remainder on the right is its self time — the same reading a sampled flame
   * graph gives, arrived at from exact durations rather than from counting samples.
   */
  function layoutFlameMerged(roots, total, span) {
    var drawn = [];
    var maxDepth = 0;
    var stack = [];

    var pen = 0;
    roots.sort(function (a, b) { return b.shown - a.shown; });
    for (var r = 0; r < roots.length; r++) {
      roots[r].x = FLOW_PAD + pen;
      roots[r].w = total > 0 ? (roots[r].shown / total) * span : span;
      pen += roots[r].w;
      stack.push(roots[r]);
    }
    ...
    while (stack.length) {
      ...
      f.kids.sort(function (a, b) { return b.shown - a.shown; });
      var cx = f.x;
      for (var k = 0; k < f.kids.length; k++) {
        var kid = f.kids[k];
        var kw = f.shown > 0 ? (kid.shown / f.shown) * f.w : 0;
        if (kw < FLAME_MIN_PX) continue;   // too thin to see or hit; its subtree goes too
        kid.x = cx;
        kid.w = kw;
        cx += kw;
        stack.push(kid);
    ...
    return { drawn: drawn, maxDepth: maxDepth };
  }

  /** The merged frame a path points at, or null once a filter has removed it. */
  function findFlameFrame(roots, path) {
    var stack = roots.slice();
    while (stack.length) {
      var f = stack.pop();
      if (f.path === path) return f;
      if (path.indexOf(f.path) === 0) {
        for (var i = 0; i < f.kids.length; i++) stack.push(f.kids[i]);
      }
    }
    return null;
  }
  ...
  /**
   * The "callers of" sandwich view: every occurrence of one method or query anywhere in the
   * run, merged into a single frame at the base, with its callers merged by identity and
   * stacked above it one ring per step up the call stack &mdash; the mirror image of {@link
   * buildFlameMerged}, walking each match's {@code parentSeq} chain outward instead of
   * walking the whole run's children inward.
   *
   * <p>A frame built here duck-types the same shape {@link buildFlameMerged} produces
   * ({@code kids}/{@code parent}/{@code path}/{@code shown}/{@code calls}), so {@link
   * layoutFlameMerged} and {@code drawNode} need no changes to lay this out and paint it: as
   * far as either is concerned this is just another merged flame with one root.
   *
   * @return {{roots: Array, total: number}}, {@code roots} empty once nothing in the run
   *   still matches {@code targetKey} (a file or SQL filter can remove every instance).
   */
  function buildFlameCallers(targetKey) {
    var searching = flowFilterText.trim() !== '' && flowMatchSelf;
    var root = null;
    var total = 0;
    ...
    for (var i = 0; i < flowNodes.length; i++) {
      var n = flowNodes[i];
      if (n.key !== targetKey) continue;
      if (n.cname && !classSelected(n.cname)) continue;
      if (!showExcluded && foldableSeq[n.seq]) continue;
      if (!flowSql && n.isSql) continue;

      if (!root) {
        root = flameCallersFrame(n, 0, null, n.key);
      }
      root.shown += n.total;
      root.calls++;
      if (searching && flowMatchSelf[flowIdxBySeq[n.seq]]) root.matched = true;
      total += n.total;

      // One ring per caller out from the target, merging whichever callers share an
      // identity at that ring — same rule buildFlameMerged applies going the other way.
      var frame = root;
      var path = root.path;
      for (var cur = flowBySeq[n.parentSeq]; cur; cur = flowBySeq[cur.parentSeq]) {
        path += '' + cur.key;
        var child = frame.kidByKey[cur.key];
        if (!child) {
          child = flameCallersFrame(cur, frame.depth + 1, frame, path);
          frame.kids.push(child);
          frame.kidByKey[cur.key] = child;
        }
        // Every ring out from the target carries that SAME occurrence's own time: this is
        // "how much of the target's time arrived via this lineage of callers", not the
        // caller's own total elsewhere in the run (which may include calls that never
    ...
    return { roots: root ? [root] : [], total: total };
  }

  function flameCallersFrame(n, depth, parent, path) {
    return {
      seq: n.seq, cls: n.cls, mth: n.mth, sig: n.sig, isSql: n.isSql, sql: n.sql,
      owner: n.owner, cname: n.cname, key: n.key, sqlLines: n.sqlLines,
      depth: depth, shown: 0, calls: 0, matched: false,
      kids: [], kidByKey: {}, parent: parent, path: path
    };
  }
  ...
  function flowColors() {
    ...
    return {
      ...
      // in the Flow Chart means the same thing it does everywhere else in the report.
      full: v('--full', '#2da44e'),
  ...
  var CHART_LOOP_WINDOW = 6; // longest repeating call pattern the animation folds into one lap

  /**
   * Folds a straight run of node visits down to one lap per repeating call pattern, so the
   * Play marker steps through a loop body once instead of idling through every iteration —
   * a tight loop like the same query fired 100 times back to back, or a call and its one
   * child alternating 100 times, both collapse the same way. At each position it looks for
   * the window length (1..{@link CHART_LOOP_WINDOW}) whose repeat covers the most ground,
   * emits one path entry per node in that window tagged with how many laps it ran, then
   * jumps past the whole repeated span; a span with no repeat just passes through untouched.
   */
  function collapseChartLoops(raw) {
    var out = [];
    var i = 0;
    var n = raw.length;
    while (i < n) {
      var bestLen = 1, bestReps = 1;
      var maxWindow = Math.min(CHART_LOOP_WINDOW, Math.floor((n - i) / 2));
      for (var len = 1; len <= maxWindow; len++) {
        var reps = 1;
        while (i + (reps + 1) * len <= n) {
          var same = true;
          for (var k = 0; k < len; k++) {
            if (raw[i + reps * len + k] !== raw[i + (reps - 1) * len + k]) { same = false; break; }
          }
          if (!same) break;
          reps++;
        }
        if (reps >= 2 && reps * len > bestReps * bestLen) { bestLen = len; bestReps = reps; }
      }
      if (bestReps >= 2) {
        for (var w = 0; w < bestLen; w++) out.push({ node: raw[i + w], reps: bestReps });
        i += bestLen * bestReps;
      } else {
        out.push({ node: raw[i], reps: 1 });
        i++;
      }
    }
    return out;
  ...
  /**
   * Collapses {@link flowVisible} into the Flow Chart's graph: one node per distinct
  ...
  function buildChart() {
    ...
    for (var i = 0; i < flowVisible.length; i++) {
      ...
      if (!cn) {
        ...
        cn = chartByKey[n.key] = {
          ...
          className: n.cname, methodName: raw.methodName, owner: n.owner,
          status: n.isSql ? null : rollupStatus(n.cname, raw.methodName),
          depth: n.depth, order: order, num: order + 1, firstSeq: n.seq,
      ...
      if (pn) {
        if (pn.key === n.key) {
          ...
          var ek = pn.key + '>' + n.key;
          if (!chartEdgeByKey[ek]) {
            var edge = { from: pn.key, to: n.key };
            chartEdgeByKey[ek] = edge;
            chartEdges.push(edge);
          }
        }
      }
    }
    chartPath = collapseChartLoops(rawPath);
    ...
    chartPathSampled = chartPath.length > CHART_MAX_PATH;
    if (chartPathSampled) {
      var sampled = [];
   ...
   * else. Start points at every depth-0 step (normally just the trace point); End takes
   * one edge, from whichever call actually ran last.
   *
   * <p>Every other leaf (a step nothing calls onward from) just stops: it returns to its
   * own caller, not to the end of the run, so drawing it as a shortcut into End would claim
   * a thing that isn't true — the diagram used to do exactly that, and it read as if
   * unrelated branches all fed into each other. Only the one call that was chronologically
   * last is an actual dead end for the run as a whole.
   */
  function addChartTerminals() {
    ...
    var start = { key: '__start__', term: 'start', label: 'Start', depth: -1, order: 0,
                  num: 0, n: 0, total: 0, calls: 0, isSql: false, recursive: false };
    var end = { key: '__end__', term: 'end', label: 'End', depth: maxDepth + 1, order: 0,
                num: chartNodes.length + 1, n: 0, total: 0, calls: 0, isSql: false, recursive: false };
    ...
    if (chartLastKey && chartByKey[chartLastKey]) {
      chartEdges.push({ from: chartLastKey, to: end.key });
    }
  ...
  /** The dot-separated portion of a fully qualified class name before its own, or null for
   *  a class with no package (or no name at all — a SQL node with no identifiable owner). */
  function chartPackageOf(full) {
    if (!full) return null;
    var i = full.lastIndexOf('.');
    return i > 0 ? full.slice(0, i) : null;
  }

  /** The package a node belongs to, charged to the issuing class for a SQL node the same
   *  way its colour already is — a query has no package of its own to group by. */
  function chartGroupInfo(n) {
    var pkg = chartPackageOf(n.isSql ? n.owner : n.className);
    if (!pkg) return null;
    return { key: pkg, label: pkg.slice(pkg.lastIndexOf('.') + 1) };
  }

  /** A stable hue from a string, so the same package lands on the same colour on every
   *  repaint rather than reshuffling each time the groups are recomputed. */
  function chartGroupHue(key) {
    var h = 0;
  ...
  /**
   * Buckets sibling steps that share a package into one visually clustered group, the way
   * the reference diagram bands its output heads by business area — a call flow has no
   * business-area equivalent, so package is the nearest thing that is both meaningful (it
   * usually tracks an architectural layer: repository, service, controller) and already on
   * every node for free.
   *
   * <p>Bucketed by each node's own drawn coordinate rather than by depth: a wide band in
   * vertical mode can wrap onto several visual rows, and two nodes at the same depth but a
   * different wrapped row are not actually siblings on screen, whatever the call tree says.
   * Grouping by depth there would silently merge two unrelated rows into one run.
   *
   * <p>Only a run of two or more gets a group — a lone box with nothing beside it to share
   * the package would just repeat the name already printed inside it. And only a run sitting
   * on the first visual row of its band gets the header pill above it: a wrapped later row,
   * like every column in horizontal mode, has only the tight sibling gap above it, not the
   * full gap between depths, and there is nowhere for the pill to go without overlapping the
   * row it would sit on top of. Those runs still get the softer backdrop panel, just not the
   * label.
   */
  function computeChartGroups() {
    ...
    chartNodes.forEach(function (n) {
      if (n.term) return;
      var k = chartVertical ? Math.round(n.y) : Math.round(n.x);
      (byBand[k] || (byBand[k] = [])).push(n);
    });
    var bandMinY = {};
    if (chartVertical) {
      chartNodes.forEach(function (n) {
        if (n.term) return;
        if (bandMinY[n.depth] == null || n.y < bandMinY[n.depth]) bandMinY[n.depth] = n.y;
      });
    }
    Object.keys(byBand).forEach(function (k) {
      var list = byBand[k].slice().sort(function (a, b) {
        return chartVertical ? a.x - b.x : a.y - b.y;
      });
      var i = 0;
      while (i < list.length) {
        var info = chartGroupInfo(list[i]);
        var j = i + 1;
        if (info) {
          while (j < list.length) {
            var infoJ = chartGroupInfo(list[j]);
            if (!infoJ || infoJ.key !== info.key) break;
            j++;
          }
        }
        if (info && j - i >= 2) {
          var run = list.slice(i, j);
          var xs = run.map(function (n) { return n.x; });
          var x2s = run.map(function (n) { return n.x + n.w; });
          var ys = run.map(function (n) { return n.y; });
          var y2s = run.map(function (n) { return n.y + n.h; });
          var y0 = Math.min.apply(null, ys);
          chartGroups.push({
            label: info.label, hue: chartGroupHue(info.key),
            x0: Math.min.apply(null, xs), x1: Math.max.apply(null, x2s),
            y0: y0, y1: Math.max.apply(null, y2s),
            pill: chartVertical && y0 === bandMinY[run[0].depth]
          });
        }
        i = j;
      }
  ...
  /** A soft package-tinted panel behind a run of grouped boxes, with a small pill label
   *  above it when there is room — see {@link computeChartGroups} for when there isn't. */
  function drawChartGroup(ctx, c, g) {
    var pad = 8;
    var bx = g.x0 - pad, by = g.y0 - pad;
    var bw = (g.x1 - g.x0) + pad * 2, bh = (g.y1 - g.y0) + pad * 2;
    roundRect(ctx, bx, by, bw, bh, 14);
    ctx.fillStyle = 'hsl(' + g.hue + ',50%,' + (c.dark ? '17%' : '95%') + ')';
    ctx.fill();
    if (!g.pill) return;
    ctx.font = 'bold 9px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';
    var label = g.label.toUpperCase();
    var lw = ctx.measureText(label).width;
    var pillW = lw + 18, pillH = 17;
    var pillX = bx + bw / 2 - pillW / 2, pillY = by - pillH - 7;
    roundRect(ctx, pillX, pillY, pillW, pillH, pillH / 2);
    ctx.fillStyle = 'hsl(' + g.hue + ',55%,' + (c.dark ? '25%' : '89%') + ')';
    ctx.fill();
    ctx.fillStyle = 'hsl(' + g.hue + ',60%,' + (c.dark ? '74%' : '36%') + ')';
    ctx.fillText(label, pillX + 9, pillY + pillH / 2 + 1);
    ctx.font = FLOW_FONT;
  }
  ...
  function layoutChartHorizontal() {
    ...
    colDepths.forEach(function (depth) {
      var list = cols[depth].sort(chartSiblingCompare);
      var y = CHART_PAD;
    ...
    var railY = maxBottom + CHART_PAD;
    chartContentH = railY + CHART_PAD;
    var end = chartByKey['__end__'];
    chartEdges.forEach(function (e) {
      var a = chartByKey[e.from];
      var b = chartByKey[e.to];
      if (!a || !b) return;
      e.x1 = a.term ? a.x + a.w / 2 + CHART_TERM_W / 2 : a.x + a.w;
      e.y1 = a.y + a.h / 2;
      if (end && b === end) {
        // The run's actual last call can sit in any column short of the deepest one, so a
        // direct line to End would cut across every column in between at whatever angle the
        // two centres happen to imply, crossing straight through boxes it has nothing to do
        // with. Routing it under everything instead — down, along a rail below the last
        // row, then up into End's own underside — means it only ever touches End itself.
        e.rail = true;
        e.railY = railY;
        e.x2 = end.x + end.w / 2; e.y2 = end.y + end.h;
      } else {
        e.x2 = b.x; e.y2 = b.y + b.h / 2;
      }
    });
    spreadChartEdgeAnchors();
    layoutChartEdgeBends(true);
  }
  ...
  function layoutChartVertical(avail) {
    ...
    depths.forEach(function (depth) {
      var list = bands[depth].sort(chartSiblingCompare);
    ...
    var railX = maxRowW + CHART_PAD;
    chartContentW = railX + CHART_PAD;
    ...
    chartEdges.forEach(function (e) {
      ...
      if (!a || !b) return;
      if (end && b === end) {
        // Same reasoning as the horizontal layout's rail (see layoutChartHorizontal): the
        // last call can be in any band short of the deepest one, so a direct line up there
        // would cross bands that have nothing to do with it. Out to a rail past the widest
        // row, down to End's band, then in through End's own side instead.
        e.rail = true;
        e.railX = railX;
        e.x1 = a.x + a.w; e.y1 = a.y + a.h / 2;
        e.x2 = end.x + end.w / 2 + CHART_TERM_W / 2; e.y2 = end.y + end.h / 2;
      } else {
    ...
    spreadChartEdgeAnchors();
    layoutChartEdgeBends(false);
  }

  /**
   * Ports along a box's own edge, one per outgoing (or incoming) connector, instead of
   * every edge from — or to — the same box leaving from its exact centre. A fan-out of five
   * curves that all start at one point bunches into an unreadable knot right at the box
   * before it has any room to spread; starting them already spread, in the same order
   * they'll arrive in, means no two of them have to cross just to get clear of the box they
   * share.
   *
   * <p>Runs after every edge's endpoints are set but before {@link layoutChartEdgeBends},
   * which fans the *middle* of edges crossing the same gap — a separate, complementary pass
   * that still matters once endpoints are spread, since two edges can still meet mid-span
   * on their way between two different pairs of ports.
   */
  function spreadChartEdgeAnchors() {
    // A terminator's drawn pill is narrower than the grid cell it's centred in (see
    // drawChartTerminator); ports have to sit on the pill's own visible edge; the cell's
    // edge would start every line in mid-air beside it.
    function box(n) {
      if (!n.term) return { x: n.x, y: n.y, w: n.w, h: n.h };
      var w = CHART_TERM_W, h = 34;
      return { x: n.x + n.w / 2 - w / 2, y: n.y + n.h / 2 - h / 2, w: w, h: h };
    }
    function spread(list, key, sortKey) {
      if (list.length < 2) return;
      // Ordered by where each edge's other end sits, on the cross axis, so the ports come
      // out in the same top-to-bottom (or left-to-right) order the lines will end up in —
      // otherwise spreading them out could just as easily add crossings as remove them.
      list.sort(function (p, q) { return p[sortKey] - q[sortKey]; });
      var b = box(chartByKey[list[0][key === 'x1' || key === 'y1' ? 'from' : 'to']]);
      if (!b) return;
      var size = chartVertical ? b.w : b.h;
      var span = Math.max(0, size - 10);
      var start = (chartVertical ? b.x : b.y) + (size - span) / 2;
      list.forEach(function (e, i) {
        e[key] = start + span * (i + 0.5) / list.length;
      });
    }
    var axis1 = chartVertical ? 'x1' : 'y1';
    var axis2 = chartVertical ? 'x2' : 'y2';
    var byFrom = {}, byTo = {};
    chartEdges.forEach(function (e) {
      if (e.x1 == null || e.rail) return;
      (byFrom[e.from] || (byFrom[e.from] = [])).push(e);
      (byTo[e.to] || (byTo[e.to] = [])).push(e);
    });
    Object.keys(byFrom).forEach(function (k) { spread(byFrom[k], axis1, axis2); });
    Object.keys(byTo).forEach(function (k) { spread(byTo[k], axis2, axis1); });
  }

  /**
   * Every edge crossing the same column gap (horizontal layout) or band gap (vertical)
   * used to bend at the exact same shared midpoint, so a handful of sibling calls into the
   * next column stacked their elbows on one vertical line and read as a single connector
   * running past boxes it never actually touched. Edges are grouped by the gap they cross
   * and each gets its own lane spaced across that gap, ordered by the other axis so lanes
   * that are already close together don't cross more than they have to.
   */
  function layoutChartEdgeBends(horizontal) {
    var groups = {};
    chartEdges.forEach(function (e) {
      if (e.x1 == null || e.rail) return;
      var key = horizontal
        ? Math.round(e.x1) + '|' + Math.round(e.x2)
        : Math.round(e.y1) + '|' + Math.round(e.y2);
    ...
    Object.keys(groups).forEach(function (key) {
      ...
      if (list.length === 1) {
        var only = list[0];
        only.bend = horizontal ? (only.x1 + only.x2) / 2 : (only.y1 + only.y2) / 2;
        return;
      }
      list.sort(function (p, q) {
        return horizontal ? ((p.y1 + p.y2) - (q.y1 + q.y2)) : ((p.x1 + p.x2) - (q.x1 + q.x2));
      });
      list.forEach(function (e, i) {
        var frac = (i + 1) / (list.length + 1);
        e.bend = horizontal ? e.x1 + frac * (e.x2 - e.x1) : e.y1 + frac * (e.y2 - e.y1);
      });
    });
  }
  ...
  /**
   * Random warm fills for flame frames, the way Brendan Gregg's original flamegraph.pl
   * colours a CPU flame graph: every frame gets its own independently rolled hue in the
   * hot range (high red, wandering green, almost no blue) rather than a colour tied to
   * its identity. Two calls to the same method land on two different fills, on purpose —
   * width and position carry identity here, not colour.
   *
   * <p>Seeded off the call's own sequence number so the same frame paints the same colour
   * on every repaint, scroll and PNG export, instead of flickering on each redraw the way
   * an unseeded Math.random() would.
   *
   * <p>SQL keeps a fill of its own, deliberately the only cool colour on the canvas:
   * telling a query apart from a method is the one distinction this diagram cannot afford
   * to lose, and a warm ramp has no shade left over that could carry it.
   */
  var FLAME_SQL_LIGHT = '#3d6ea8';
  var FLAME_SQL_DARK = '#6f9fdd';
  var FLAME_MATCH_LIGHT = '#e619b0';   // classic flamegraph.pl search magenta
  var FLAME_MATCH_DARK = '#ff5fd6';

  /** A tiny seeded PRNG (mulberry32), so a colour rolled once stays put across repaints. */
  ...
  /** One independently-rolled warm rgb() per call, cached by seq so it never re-rolls. */
  function flameFrameColor(seq) {
    var hit = flameColorCache[seq];
    if (hit) return hit;
    var rnd = mulberry32(seq + 1);
    var r = 205 + Math.floor(50 * rnd());
    var g = Math.floor(230 * rnd());
    var b = Math.floor(55 * rnd());
    return flameColorCache[seq] = 'rgb(' + r + ',' + g + ',' + b + ')';
  }

  /** The package a class belongs to, which is what "whole package" search scope matches on. */
  function packageOf(fq) {
    var i = String(fq || '').lastIndexOf('.');
    return i > 0 ? fq.slice(0, i) : (fq || '(default)');
  }

  /** Re-expresses a fill as translucent, to dim whatever the active search didn't match. */
   ...
   * white per box, which is the only way one text rule works across every random fill.
   */
  function inkOn(fill) {
    var r, g, b;
  ...
  function drawFlow() {
    ...
    } else if (flowFlame && flameCallersTarget) {
      var builtC = buildFlameCallers(flameCallersTarget);
      if (builtC.roots.length === 0) {
        // The targeted method or query was filtered out from under us (the file picker, the
        // SQL toggle) and no longer matches anything; there is nothing left to show callers
        // of, so fall back the same way a zoom does when its own frame disappears.
        flameCallersTarget = null;
        flameCallersArm = false;
        syncFlowSubViewControls();
        drawFlow();
        return;
      }
      var cSpan = flowFit ? (avail - FLOW_PAD * 2)
        : Math.max(FLOW_FLAME_W, avail - FLOW_PAD * 2) * flowZoom;
      var laidC = layoutFlameMerged(builtC.roots, builtC.total || 1, cSpan);
      flameWidened = false;
      laidC.drawn.forEach(function (n) {
        var row = laidC.maxDepth - n.depth;
        n.y = FLOW_PAD + FLOW_RULER + row * FLAME_BOX;
        n.h = FLAME_BOX;
        (flowByDepth[row] || (flowByDepth[row] = [])).push(n);
      });
      flowDrawn = laidC.drawn;
      flameMergedTotal = builtC.total;
      contentW = FLOW_PAD * 2 + cSpan;
      contentH = FLOW_PAD * 2 + FLOW_RULER + (laidC.maxDepth + 1) * FLAME_BOX;
    } else if (flowFlame && flameMerged) {
      var built = buildFlameMerged();
      var mRoots = built.roots;
      var mTotal = built.total;

      // Zooming makes one frame the new 100%: the same rule as the chronological view,
      // except the frame is found by its call path rather than by a recorded seq, since a
      // merged frame is many calls and has no single one of its own.
      if (flameRootPath) {
        var focus = findFlameFrame(mRoots, flameRootPath);
        if (!focus) {
          flameRootPath = null;      // filtered out from under us
        } else {
          focus.parent = null;       // it is the root of the drawn tree now
          mRoots = [focus];
          mTotal = focus.shown || 1;
        }
      }

      var mSpan = flowFit ? (avail - FLOW_PAD * 2)
        : Math.max(FLOW_FLAME_W, avail - FLOW_PAD * 2) * flowZoom;
      var laid = layoutFlameMerged(mRoots, mTotal, mSpan);
      // Nothing is ever widened past its share here, so the note has no caveat to make.
      flameWidened = false;
      laid.drawn.forEach(function (n) {
        var row = laid.maxDepth - n.depth;
        n.y = FLOW_PAD + FLOW_RULER + row * FLAME_BOX;
        n.h = FLAME_BOX;
        (flowByDepth[row] || (flowByDepth[row] = [])).push(n);
      });
      flowDrawn = laid.drawn;
      flameMergedTotal = mTotal;
      contentW = FLOW_PAD * 2 + mSpan;
      contentH = FLOW_PAD * 2 + FLOW_RULER + (laid.maxDepth + 1) * FLAME_BOX;
    } else if (flowFlame) {
    ...
    if (tab === 'flow') {
      countEl.textContent = flowChart
        ...
        // Merged frames are not steps and there is no "of" to give them: every call is on
        // the diagram, just not in a box of its own.
        : (flowFlame && flameMerged)
          ? flowDrawn.length + (flowDrawn.length === 1 ? ' frame, ' : ' frames, ')
            + flowNodes.length + ' calls'
  ...
  function paintFlow() {
    ...
    paintMinimap(sx, sy, viewW, viewH);
  }

  var MINIMAP_W = 190, MINIMAP_H = 130, MINIMAP_PAD = 10, MINIMAP_MARGIN = 3;

  /**
   * A small overview of the whole Flow Chart in the corner, with a rectangle marking what's
   * currently on screen &mdash; orientation for a diagram too large to see all of at once,
   * the way an image editor's navigator panel works.
   *
   * <p>Reads only what {@link paintFlow} and {@link drawFlow} already computed
   * ({@link chartNodes}, {@link flowContentW}/{@link flowContentH}, {@link flowScale}) and
   * draws to its own canvas; nothing about the chart's own layout, edges or paint path
   * changes because this exists.
   *
   * <p>Pinned over a corner of the viewport the same way {@link flowCanvas} is: it is a
   * child of the scrolling spacer, not the wrap, so its {@code left}/{@code top} are
   * recomputed here on every scroll rather than left to CSS, which has no "stay in the
   * corner of this scrolled box" position of its own.
   */
  function paintMinimap(sx, sy, viewW, viewH) {
    if (!flowMinimap) return;
    if (!chartMinimapOn || !flowChart || flowContentW === 0) {
      flowMinimap.style.display = 'none';
      return;
    }
    flowMinimap.style.display = 'block';
    flowMinimap.style.left = (sx + viewW - MINIMAP_W - MINIMAP_PAD) + 'px';
    flowMinimap.style.top = (sy + viewH - MINIMAP_H - MINIMAP_PAD) + 'px';

    var dpr = flowDpr();
    var needW = Math.ceil(MINIMAP_W * dpr);
    var needH = Math.ceil(MINIMAP_H * dpr);
    if (flowMinimap.width !== needW || flowMinimap.height !== needH) {
      flowMinimap.width = needW;
      flowMinimap.height = needH;
      flowMinimap.style.width = MINIMAP_W + 'px';
      flowMinimap.style.height = MINIMAP_H + 'px';
    }
    var ctx = flowMinimap.getContext('2d');
    ...
    var ox = MINIMAP_MARGIN, oy = MINIMAP_MARGIN;
    chartNodes.forEach(function (n) {
      if (n.w == null) return;
      ctx.globalAlpha = n.term ? 0.55 : 0.8;
      ctx.fillStyle = n.term ? c.muted : (n.isSql ? c.sql : c.accent);
      ctx.fillRect(ox + n.x * scale, oy + n.y * scale,
        Math.max(1, n.w * scale), Math.max(1, n.h * scale));
    });
    ctx.globalAlpha = 1;

    // The part of the diagram actually on screen right now, in the same content-space
    // rectangle paintFlow just handed renderFlow.
    ctx.strokeStyle = c.fg;
    ctx.lineWidth = 1.5;
    ctx.strokeRect(
      ox + (sx / flowScale) * scale, oy + (sy / flowScale) * scale,
      (viewW / flowScale) * scale, (viewH / flowScale) * scale);
  ...
  function minimapScale() {
    return Math.min((MINIMAP_W - MINIMAP_MARGIN * 2) / flowContentW,
                     (MINIMAP_H - MINIMAP_MARGIN * 2) / flowContentH);
  }

  /** Scrolls the main diagram to center on wherever the minimap was clicked or dragged. */
  function jumpToMinimapPoint(e) {
    if (!flowMinimap || flowContentW === 0) return;
    var r = flowMinimap.getBoundingClientRect();
    var scale = minimapScale();
    var cx = (e.clientX - r.left - MINIMAP_MARGIN) / scale;
    var cy = (e.clientY - r.top - MINIMAP_MARGIN) / scale;
    var viewW = flowWrap.clientWidth || 1;
    var viewH = flowWrap.clientHeight || 1;
    flowWrap.scrollLeft = Math.max(0, cx * flowScale - viewW / 2);
    flowWrap.scrollTop = Math.max(0, cy * flowScale - viewH / 2);
  }
  ...
  function renderFlow(ctx, x0, y0, x1, y1) {
    ...
    if (flowFlame) {
      // The pale grey-to-yellow wash a classic CPU flame graph paints behind its stack.
      // Built against the diagram's own coordinates, not the viewport slice being painted,
      // so the wash stays put as the canvas scrolls instead of sliding under it.
      //
      // Flat when there is no gradient to be had: the SVG export runs this same renderer
      // through a context shim that implements paths, fills and text and nothing else, so
      // asking it for a gradient threw and took the whole export with it. One colour from
      // the middle of the wash is a fair stand-in, and a saved file beats an exact one
      // that never arrives.
      if (ctx.createLinearGradient) {
        var grad = ctx.createLinearGradient(0, 0, 0, flowContentH);
        if (c.dark) {
          grad.addColorStop(0, '#242322');
          grad.addColorStop(1, '#2e2a1c');
        } else {
          grad.addColorStop(0, '#eeeeee');
          grad.addColorStop(1, '#eeeeb0');
        }
        ctx.fillStyle = grad;
      } else {
        ctx.fillStyle = c.dark ? '#292724' : '#eeeed7';
      }
    } else {
      ctx.fillStyle = c.bg;
    }
    ...
    if (flowFlame) {
      // No time ruler: a classic flame graph reads width as a share of what's beside it,
      // not a position against a clock, so a ms axis would claim a precision the reference
      // deliberately leaves out. The title takes the same band instead.
      if (y0 < FLOW_PAD + FLOW_RULER) {
        ctx.font = '600 12px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';
        ctx.fillStyle = c.muted;
        ctx.textAlign = 'center';
        ctx.fillText('Flame Graph', (x0 + x1) / 2, FLOW_PAD + FLOW_RULER / 2);
        ctx.textAlign = 'left';
        ctx.font = FLOW_FONT;
      }
  ...
  function drawNode(ctx, c, n) {
    ...
    if (flowFlame) {
      var searching = flowFilterText.trim() !== '' && flowMatchSelf;
      // A merged frame carries its own answer: it stands for many calls, and they all
      // share the signature the pattern is tested against, so it was settled when the
      // frame was built rather than looked up per repaint.
      var isMatch = searching
        && (n.matched !== undefined ? n.matched : flowMatchSelf[flowIdxBySeq[n.seq]]);
      var fill;
      if (isMatch) {
        fill = c.dark ? FLAME_MATCH_DARK : FLAME_MATCH_LIGHT;
      } else {
        fill = n.isSql ? (c.dark ? FLAME_SQL_DARK : FLAME_SQL_LIGHT) : flameFrameColor(n.seq);
        if (searching) fill = withAlpha(fill, 0.35);
      }
      ctx.fillStyle = fill;
      ctx.fill();
      ink = searching && !isMatch ? withAlpha(inkOn(fill), 0.5) : inkOn(fill);
    }
    if (over) {
    ...
    // A narrow flame frame carries only a name, the way a classic flame graph reads: width
    // already says how long the call took, and there isn't room for numbers besides. Once a
    // frame is wide enough to spare it, it gets its time and share too — otherwise that pair
    // is one hover away in the tooltip and nowhere else, which meant comparing two boxes
    // without a mouse was never possible.
    if (!flowFlame || n.w >= FLAME_METRICS_MIN_W) {
      ...
      if (flowFlame) {
        var pctTxt = pct(n.shown);
        if (pctTxt) {
          var pctW = ctx.measureText(pctTxt).width;
          ctx.fillStyle = ink || c.muted;
          ctx.fillText(pctTxt, right - pctW, mid);
          right -= pctW + 6;
        }
      }
  ...
  function renderChartFrame(ctx, c, x0, y0, x1, y1) {
    // Group backdrops sit behind everything else — connectors, boxes, marker — so they
    // read as a panel the steps are clustered on top of rather than a layer painted over
    // them.
    for (var g = 0; g < chartGroups.length; g++) {
      var grp = chartGroups[g];
      if (grp.x1 < x0 || grp.x0 > x1 || grp.y1 < y0 || grp.y0 > y1) continue;
      drawChartGroup(ctx, c, grp);
    }
    ctx.font = FLOW_FONT;
    // Connectors get the stronger --muted grey rather than the very pale --border one
    // everything else uses for hairlines: a directional path someone has to trace by eye
    // needs more contrast than a table rule does.
    ctx.strokeStyle = c.muted;
    ctx.lineWidth = 1.5;
    for (var i = 0; i < chartEdges.length; i++) {
      var e = chartEdges[i];
      if (e.x1 == null) continue;
      var exHi = Math.max(e.x1, e.x2), eyHi = Math.max(e.y1, e.y2);
      if (e.rail) { if (chartVertical) exHi = Math.max(exHi, e.railX); else eyHi = Math.max(eyHi, e.railY); }
      if (exHi < x0 || Math.min(e.x1, e.x2) > x1) continue;
      if (eyHi < y0 || Math.min(e.y1, e.y2) > y1) continue;
  ...
  /**
   * A caller-to-callee connector, curved smoothly through the gap between one depth and the
   * next rather than elbowed at a sharp right angle — the flowing S-curve a real diagramming
   * tool draws between two boxes, not a circuit-board trace. Horizontal mode bows the curve
   * vertically (bulging up or down) since depth runs left to right there; vertical mode bows
   * it horizontally instead, since depth runs top to bottom. Either way the curve leaves and
   * arrives perpendicular to the box faces it connects, so the arrowhead always ends up
   * pointing straight into whichever side of the callee's box actually faces its caller.
   */
  var CHART_CURVE_SAMPLES = 16;

  /**
   * How far each control point leans toward the *other* endpoint's position on the free
   * axis, instead of sitting exactly on its own endpoint's. At 0 every edge leaving the same
   * box starts out moving in exactly the same direction — flat along the bend axis — no
   * matter how far apart the ports it left from are, so a fan-out reads as one bundle of
   * lines that only pulls apart once it's most of the way across the gap. Leaning the control
   * points toward each other's endpoint instead means the curve is already peeling off toward
   * its own target from the moment it leaves the box, which is what actually keeps a bundle
   * of five readable as five lines rather than one for most of their length.
   */
  var CHART_CURVE_LEAN = 0.4;

  /**
   * Points along a cubic Bezier from {@code e}'s start to its end, control points pulled out
   * to the edge's bend axis so the curve reads as one flowing line rather than a straight
   * shortcut through whatever sits between the two boxes. Sampled into a fine polyline —
   * not drawn with a real bezierCurveTo — because the SVG export runs every draw call
   * through a context shim (see {@link svgContext}) that implements straight and arc
   * segments and nothing else; a sampled curve degrades to an very-close polyline there
   * instead of throwing away the export entirely.
   *
   * <p>Which axis the control points sit on flips for a rail edge (the detour a leaf branch
   * takes around a busy row to reach the synthetic End node): an ordinary edge in vertical
   * mode bows sideways (fixed x, bend is a y), while a rail edge in that same mode bows
   * up/down instead (fixed y, bend is an x) because it is routing around a row rather than
   * through the normal gap between two depths. Horizontal mode is the mirror image of both.
   */
  function chartEdgePoints(e) {
    var x1 = e.x1, y1 = e.y1, x2 = e.x2, y2 = e.y2;
    var yBend = chartVertical !== !!e.rail;
    var bend = e.rail ? (yBend ? e.railY : e.railX)
      : (e.bend != null ? e.bend : (yBend ? (y1 + y2) / 2 : (x1 + x2) / 2));
    var c1x, c1y, c2x, c2y;
    if (yBend) {
      c1x = x1 + (x2 - x1) * CHART_CURVE_LEAN; c1y = bend;
      c2x = x2 - (x2 - x1) * CHART_CURVE_LEAN; c2y = bend;
    } else {
      c1x = bend; c1y = y1 + (y2 - y1) * CHART_CURVE_LEAN;
      c2x = bend; c2y = y2 - (y2 - y1) * CHART_CURVE_LEAN;
    }
    var pts = [];
    for (var i = 0; i <= CHART_CURVE_SAMPLES; i++) {
      var t = i / CHART_CURVE_SAMPLES;
      var mt = 1 - t;
      var a = mt * mt * mt, b = 3 * mt * mt * t, cc = 3 * mt * t * t, d = t * t * t;
      pts.push({
        x: a * x1 + b * c1x + cc * c2x + d * x2,
        y: a * y1 + b * c1y + cc * c2y + d * y2
      });
    }
    return pts;
  }

  /** A fine polyline stroked as one continuous line, standing in for the curve it samples. */
  function curvedEdgePath(ctx, pts) {
    ctx.beginPath();
  ...
  function arrowhead(ctx, c, tipX, tipY, dx, dy) {
    var ah = 5;
    ctx.beginPath();
    ctx.moveTo(tipX, tipY);
    ctx.lineTo(tipX - dx * ah + dy * ah, tipY - dy * ah - dx * ah);
    ctx.lineTo(tipX - dx * ah - dy * ah, tipY - dy * ah + dx * ah);
    ctx.closePath();
    ctx.fillStyle = c.muted;
    ctx.fill();
  ...
  /**
   * The direction the curve is actually travelling as it reaches its last point, from the
   * sampled polyline's own final segment rather than a fixed per-orientation guess. Leaning
   * the control points toward each other (see {@link CHART_CURVE_LEAN}) means the curve no
   * longer arrives perfectly square to the box face the way the old pure elbow-style one
   * did, so the arrowhead has to read the real approach angle or it ends up twisted against
   * the line it caps.
   */
  function chartEdgeArrowDir(pts) {
  ...
  function drawChartEdge(ctx, c, e) {
    var pts = chartEdgePoints(e);
    curvedEdgePath(ctx, pts);
    var dir = chartEdgeArrowDir(pts);
    arrowhead(ctx, c, e.x2, e.y2, dir.dx, dir.dy);
  }

  /** Total length of a polyline, for placing a point a given fraction along it by arc length. */
  ...
  function pointOnPolyline(pts, t) {
    var total = polylineLen(pts);
    if (total <= 0) return pts[0];
    var target = total * Math.max(0, Math.min(1, t));
    var acc = 0;
    for (var i = 1; i < pts.length; i++) {
      var segLen = Math.hypot(pts[i].x - pts[i - 1].x, pts[i].y - pts[i - 1].y);
      if (acc + segLen >= target || i === pts.length - 1) {
        var segT = segLen > 0 ? (target - acc) / segLen : 0;
        return { x: pts[i - 1].x + (pts[i].x - pts[i - 1].x) * segT, y: pts[i - 1].y + (pts[i].y - pts[i - 1].y) * segT };
      }
      acc += segLen;
    }
  ...
  /**
   * The numbered circle every step carries, Start at 0 through End at the last number, so a
   * reader can read the run's order straight off the diagram without animating anything.
   */
  function drawChartNumBadge(ctx, c, n, bx, by) {
    var num = n.num;
    if (num == null) return;
  ...
  function drawChartTerminator(ctx, c, n) {
    ...
    var canShade = !!ctx.createLinearGradient;
    if (canShade) {
      ctx.shadowColor = c.dark ? 'rgba(0,0,0,0.45)' : 'rgba(15,23,42,0.16)';
      ctx.shadowBlur = 8;
      ctx.shadowOffsetY = 3;
    }
  ...
  function drawChartNode(ctx, c, n) {
    ...
    // The gradient and the shadow share one capability check: the SVG export runs every
    // draw call through a context shim (see {@link svgContext}) built for paths, fills and
    // text and nothing past that — no gradients, no shadow properties. Setting the shadow
    // ones there is a harmless no-op (they land on a plain object nothing reads); asking it
    // for a gradient is not, so that call is skipped there and the box falls back to a flat
    // fill instead of taking the whole export down with it.
    var canShade = !!ctx.createLinearGradient;

    // A soft drop shadow under the box, the way a card sits slightly off the page instead
    // of flush with it. Set only for this one fill — the shape's opaque base — and cleared
    // right after, since a canvas shadow follows every draw call while it is set and would
    // otherwise smear under the wash, the border and the text drawn on top of it too.
    if (canShade) {
      ctx.shadowColor = c.dark ? 'rgba(0,0,0,0.45)' : 'rgba(15,23,42,0.16)';
      ctx.shadowBlur = 8;
      ctx.shadowOffsetY = 3;
    }
    roundRect(ctx, n.x, n.y, n.w, n.h, CHART_NODE_R);
    if (canShade) {
      var grad = ctx.createLinearGradient(n.x, n.y, n.x, n.y + n.h);
      grad.addColorStop(0, c.dark ? '#2b3038' : '#ffffff');
      grad.addColorStop(1, c.dark ? '#20242b' : '#f2f4f7');
      ctx.fillStyle = grad;
    } else {
      ctx.fillStyle = c.box;
    }
    ...
    // The step's status colour, washed over the raised base rather than replacing it, so
    // the card keeps its lightly-lifted look and still reads green/amber/red/blue at a
    // glance — a pastel tint of the status colour, the way the reference's coloured head
    // cards sit on the same soft-card treatment as everything else on the diagram.
    roundRect(ctx, n.x, n.y, n.w, n.h, CHART_NODE_R);
    ...
    drawChartNumBadge(ctx, c, n, n.x, n.y);

    // Three stacked rows — class, method, metrics — instead of one crowded line: identity
    // reads top to bottom the way a stack frame does, and the bottom row is where every
    // number (time, share, repeats, recursion) lives so it can be scanned down a column of
    // boxes without ever fighting the class/method text for space. Every row is centred on
    // the box's own midline rather than pinned to either edge, so the block reads the same
    // regardless of which line happens to be the widest. textAlign is not an option: the SVG
    // export runs draw calls through a context shim that never implements it (see
    // {@link drawChartNumBadge}), so centring is done by hand from each string's measured
    // width instead of relying on the canvas to do it.
    var pad = 8;
    var cx = n.x + n.w / 2;
    var maxW = n.w - pad * 2;
    var yCls = n.y + 17;
    ...
    if (n.isSql) {
      ...
      ctx.font = FLOW_FONT;
      ctx.fillStyle = c.mth;
      centered(clip(ctx, n.mth, maxW), yMth);
    } else {
      ctx.font = 'bold 12px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';
      ctx.fillStyle = c.cls;
      centered(clip(ctx, n.cls, maxW), yCls);
      ctx.font = FLOW_FONT;
      ctx.fillStyle = c.mth;
      centered(clip(ctx, n.mth, maxW), yMth);
    }

    // Bottom row: time, then share of the run, then repeat count, then recursion — each
    // only when it has something to say, laid out left to right but centred as one group.
    ctx.font = '10px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';
    var segs = [];
    if (n.total > 0) segs.push({ text: fmt(n.total), color: c.muted });
    // Every box is the same size regardless of how long its step took, unlike the Flame
    // layout where width already says that — so the one thing this diagram cannot show by
    // shape alone, it says in text instead: the share of the whole run this step's calls
    // account for, worst offenders included even when they're a single fast-looking box.
    if (n.total > 0 && flowRootTotal > 0) {
      var pctVal = (n.total / flowRootTotal) * 100;
      var pctTxt = (pctVal >= 10 ? Math.round(pctVal) : pctVal >= 1 ? pctVal.toFixed(1)
        : pctVal >= 0.1 ? '<1' : '<0.1') + '%';
      segs.push({ text: pctTxt, color: c.muted });
    }
    if (n.n > 1) segs.push({ text: '×' + n.n, color: c.badge });
    if (n.recursive) segs.push({ text: '↻', color: c.accent });

    var gap = 6;
    var widths = segs.map(function (s) { return ctx.measureText(s.text).width; });
    var totalW = widths.reduce(function (a, b) { return a + b; }, 0) + gap * Math.max(0, segs.length - 1);
    var mx = cx - totalW / 2;
    segs.forEach(function (s, i) {
      ctx.fillStyle = s.color;
      ctx.fillText(s.text, mx, yMeta);
      mx += widths[i] + gap;
    });
    ctx.font = FLOW_FONT;
  }
  ...
  /**
   * Where the marker sits right now: parked on a node when idle, sliding along the exact
   * edge line into the next node mid-step. A step whose call site ran back-to-back several
   * times (a tight loop) is one entry in {@link chartPath} with {@code reps} > 1 — the
   * marker just parks there once rather than idling through every repeat.
   */
  function chartMarkerPos() {
    ...
    if (a.node === b.node) {
      return { x: a.node.x + a.node.w / 2, y: a.node.y + a.node.h / 2, node: a.node, reps: a.reps };
    }
    var edge = chartEdgeByKey[a.node.key + '>' + b.node.key];
    var t = chartProgress;
    if (!edge) {
      var ax = a.node.x + a.node.w / 2, ay = a.node.y + a.node.h / 2;
      var bx = b.node.x + b.node.w / 2, by = b.node.y + b.node.h / 2;
      return { x: ax + (bx - ax) * t, y: ay + (by - ay) * t, node: t < 0.5 ? a.node : b.node, reps: 1 };
    }
    var pos = pointOnPolyline(chartEdgePoints(edge), t);
    var atNode = t < 0.5 ? a.node : b.node;
    return { x: pos.x, y: pos.y, node: atNode, reps: t < 0.5 ? a.reps : b.reps };
  }
  ...
  function drawChartMarker(ctx, c) {
    ...
    ctx.lineWidth = 1;
    if (pos.reps > 1) {
      var label = pos.reps + ' times';
      ctx.font = 'bold 11px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif';
      var w = ctx.measureText(label).width;
      var lx = pos.x - w / 2, ly = pos.y - CHART_DOT_R - 16;
      roundRect(ctx, lx - 5, ly - 13, w + 10, 16, 8);
      ctx.fillStyle = c.accent;
      ctx.fill();
      ctx.fillStyle = c.bg;
      ctx.textAlign = 'center';
      ctx.fillText(label, pos.x, ly - 5);
      ctx.textAlign = 'left';
    }
  }
  ...
  function onFlowClick(e) {
    ...
    if (flowFlame) {
      if (flameCallersArm) {
        // The button armed a pick; this click is the pick, not a normal zoom. Any zoom
        // from before still has to go: it addresses a frame by seq/path, which the
        // callers view's own root (identity-merged, no single seq of its own) cannot
        // honour, and drawFlow ignores it anyway once flameCallersTarget takes over.
        flameCallersArm = false;
        flameRoot = null;
        flameRootPath = null;
        flameCallersTarget = n.key;
        flameCallersBtn.classList.add('on');   // now "on" means showing a callers view
        flameCallersBtn.textContent = 'Callers of ' + (n.isSql ? sqlLabel(n.sql) : n.cls + n.mth);
        syncFlowSubViewControls();
        hideFlowTip();
        drawFlow();
        return;
      }
      if (flameCallersTarget) {
        // Already in a callers view: click any caller ring to re-target the same view on
        // it, walking one caller at a time without leaving the view to restart it.
        flameCallersTarget = n.key;
        flameCallersBtn.textContent = 'Callers of ' + (n.isSql ? sqlLabel(n.sql) : n.cls + n.mth);
        hideFlowTip();
        drawFlow();
      ...
      if (flameMerged) {
        flameRootPath = (flameRootPath === n.path)
          ? (n.parent ? n.parent.path : null)
          : n.path;
      } else {
  ...
  function renderCrumb() {
    var focus = flameMerged ? flameRootPath : flameRoot;
    var key = (flowFlame ? 'f' : 't') + ':' + (flameMerged ? 'm' : 'c') + ':' + focus;
    if (key === crumbKey) return;
    crumbKey = key;
    if (!flowFlame || focus == null) {
      flowCrumb.hidden = true;
      flowCrumb.textContent = '';
      return;
    }

    // The chain is walked the same way in both layouts, just over different parents: a
    // recorded call's parentSeq, or a merged frame's own parent link.
    var chain = [];
    var n;
    if (flameMerged) {
      for (n = findFlameFrame(buildFlameMerged().roots, flameRootPath); n; n = n.parent) {
        chain.unshift(n);
      }
    } else {
      for (n = flowBySeq[flameRoot]; n; n = flowBySeq[n.parentSeq]) chain.unshift(n);
    }
    ...
    chain.forEach(function (f, i) {
      ...
      flowCrumb.appendChild(crumbBtn(
        f.isSql ? sqlLabel(f.sql) : (f.cls + f.mth),
        flameMerged ? f.path : f.seq, i === chain.length - 1));
    });
  ...
  function crumbBtn(label, target, current) {
    ...
    if (!current) {
      b.title = target == null ? 'Back to the whole run' : 'Zoom out to ' + label;
      b.addEventListener('click', function () {
        if (flameMerged) flameRootPath = target;
        else flameRoot = target;
        hideFlowTip();
        drawFlow();
      });
    }
  ...
  function onChartMove(n, e) {
    ...
    if (n.num != null) facts.push('step ' + n.num + ' of the run');
  ...
  function onFlowMove(e) {
    ...
    // A merged frame is not a step: it stands for every call that arrived here by the same
    // route, so it is described by how many of them there were and what they cost together,
    // and a step number would be picking one of them arbitrarily.
    var sub;
    if (n.calls !== undefined) {
      sub = (n.calls === 1 ? '1 call' : n.calls + ' calls merged') + ' · ' + fmtPct(n.shown);
    } else {
      sub = 'step ' + (n.seq + 1) + ' · ' + fmtPct(n.shown);
      if (n.reps > 1) sub += ' · ' + n.reps + ' identical calls combined';
      if (n.site != null) sub += ' · called at line ' + n.site;
    }
    if (flowFlame) {
      if (flameCallersArm) {
        sub += ' · click to see everything that calls this';
      } else if (flameCallersTarget) {
        sub += n.key === flameCallersTarget
          ? ' · this is what the view is centred on'
          : ' · click to see callers of this instead';
      } else {
        var here = flameMerged ? (flameRootPath === n.path) : (flameRoot === n.seq);
        sub += here ? ' · click to zoom out' : ' · click to zoom in';
      }
    }
  ...
  /**
   * The flow tab has three mutually exclusive layouts (plain nesting, Flame, Flow Chart),
   * and several controls only mean something in one of them: Hot path has nothing to
   * highlight in the Flow Chart (it colours by coverage, not by the tree drawNode()
   * walks), and frame colour/minimum width are meaningless outside Flame, whose boxes are
   * the only ones sized by duration. Leaving them clickable in a layout where they do
   * nothing invites exactly the "why doesn't this button do anything" confusion it caused.
   */
  function syncFlowSubViewControls() {
    flowHotBtn.hidden = flowChart;
    // A caller view is inherently merged, so Merge repeats and its width floor have
    // nothing of their own to offer while one is showing.
    flameMergeBtn.hidden = !flowFlame || !!flameCallersTarget;
    // The width floor exists to rescue sub-pixel frames from a per-call layout. Merging
    // removes them at the source, so the control has nothing left to do and would only
    // offer to make the widths dishonest again.
    if (flowMinWSel) flowMinWSel.hidden = !flowFlame || flameMerged || !!flameCallersTarget;
    // Grouping a run of identical neighbours is what merging already does, and more of it.
    flowGroupBtn.hidden = flowFlame && (flameMerged || !!flameCallersTarget);
    if (flameCallersBtn) flameCallersBtn.hidden = !flowFlame;
    if (chartMinimapBtn) chartMinimapBtn.hidden = !flowChart;
  }

  flameMergeBtn.addEventListener('click', function () {
    flameMerged = !flameMerged;
    flameMergeBtn.classList.toggle('on', flameMerged);
    // The two layouts address frames differently — a call path, or one recorded call — so
    // a zoom cannot be carried across and is dropped rather than silently pointed at the
    // wrong frame.
    flameRoot = null;
    flameRootPath = null;
    syncFlowSubViewControls();
  ...
  if (flameCallersBtn) {
    flameCallersBtn.addEventListener('click', function () {
      if (flameCallersTarget) {
        // Already showing a callers view: the button's job now is to leave it.
        flameCallersTarget = null;
        flameCallersArm = false;
        flameCallersBtn.classList.remove('on');
        flameCallersBtn.textContent = 'Callers of…';
        syncFlowSubViewControls();
        hideFlowTip();
        drawFlow();
        return;
      }
      flameCallersArm = !flameCallersArm;
      flameCallersBtn.classList.toggle('on', flameCallersArm);
      flameCallersBtn.textContent = flameCallersArm ? 'Click a frame…' : 'Callers of…';
    });
  }

  if (chartMinimapBtn) {
    chartMinimapBtn.addEventListener('click', function () {
      chartMinimapOn = !chartMinimapOn;
      chartMinimapBtn.classList.toggle('on', chartMinimapOn);
      paintMinimap();
    });
  }
  ...
  /**
   * The three layouts (Steps, Flame, Flow Chart) as one radio choice, driven by a sub-tab
   * row the same way the top-level tabs work — a click always selects a view rather than
   * toggling one flag independently of the other two, which is what let Flame and Flow
   * Chart both end up on at once back when they were separate toggle buttons.
   */
  var FLOW_VIEW_BTNS = { steps: flowStepsBtn, flame: flowFlameBtn, chart: flowChartBtn };
  function setFlowView(view) {
    flowFlame = view === 'flame';
    flowChart = view === 'chart';
    if (!flowFlame) {
      flameRoot = null;
      flameRootPath = null;
      flameCallersArm = false;
    ...
    Object.keys(FLOW_VIEW_BTNS).forEach(function (v) {
      var on = v === view;
      FLOW_VIEW_BTNS[v].classList.toggle('on', on);
      FLOW_VIEW_BTNS[v].setAttribute('aria-selected', String(on));
    });
    // Each layout has its own shape and height. A scroll position left over from whichever
    // one was on screen before can sit well past the new one's much shorter content — Steps
    // on a large run scrolls for pages, Flame is a few hundred pixels tall — and painting
    // only what is "on screen" at that stale offset then draws nothing at all.
    flowWrap.scrollTop = 0;
    flowWrap.scrollLeft = 0;
    stopChartAnim();
    chartPlayIdx = 0;
    chartProgress = 0;
    chartOrientBtn.hidden = !flowChart;
    chartLayerBtn.hidden = !flowChart;
    chartPlaySeg.hidden = !flowChart;
    chartLegend.hidden = !flowChart;
    chartSpeedSel.hidden = !flowChart;
    chartSpeedCustom.hidden = !flowChart || chartSpeedSel.value !== 'custom';
    chartSpeedVal.hidden = chartSpeedCustom.hidden;
    syncFlowSubViewControls();
    hideFlowTip();
    drawFlow();
  }
  flowStepsBtn.addEventListener('click', function () { setFlowView('steps'); });
  flowFlameBtn.addEventListener('click', function () { setFlowView('flame'); });
  flowChartBtn.addEventListener('click', function () { setFlowView('chart'); });
  ...
  chartLayerBtn.addEventListener('click', function () {
    chartSortByLayer = !chartSortByLayer;
    chartLayerBtn.classList.toggle('on', chartSortByLayer);
    chartLayerBtn.textContent = chartSortByLayer ? 'Sort: by layer' : 'Sort: call order';
    hideFlowTip();
    drawFlow();
  });
  ...
  /** Applies an absolute zoom percentage. Zoom and fit are two answers to the same
   *  question, so setting one drops the other rather than compounding into a scale
   *  nobody chose. */
  function setZoomPct(pct) {
    flowFit = false;
    flowFitBtn.classList.remove('on');
    flowZoom = Math.max(ZOOM_MIN, Math.min(ZOOM_MAX, pct / 100));
    hideFlowTip();
    drawFlow();
  }

  /** Keeps the slider and the typed box reading whatever the diagram is actually drawn
   *  at, including when Fit width or the Flame layout are the ones choosing the scale
   *  rather than a value either control set. Skips the text box while it has focus, so
   *  a redraw mid-edit (e.g. from a live slider drag) can't clobber what's being typed.
   *
   *  <p>Flame is the exception: its canvas transform stays pinned at 1 always (that's
   *  what keeps its labels legible while zoomed out), so reading {@link flowScale} there
   *  would show "100" no matter what zoom was actually applied. Flame's real zoom lives
   *  in {@code flowZoom} instead, spent on the time axis rather than the canvas. */
  function syncZoomControls() {
    var pct = flowFlame ? (flowFit ? 100 : Math.round(flowZoom * 100))
      : Math.round(flowScale * 100);
    flowZoomSlider.value = Math.max(40, Math.min(300, pct));
    if (document.activeElement !== flowZoomValue) flowZoomValue.value = pct;
  }

  flowZoomSlider.addEventListener('input', function () {
    setZoomPct(parseFloat(flowZoomSlider.value));
  });

  function applyZoomValueInput() {
    var v = parseFloat(flowZoomValue.value);
    if (isFinite(v) && v > 0) setZoomPct(v);
    else syncZoomControls();
  }
  flowZoomValue.addEventListener('keydown', function (e) {
    if (e.key === 'Enter') { applyZoomValueInput(); flowZoomValue.blur(); }
    else if (e.key === 'Escape') { syncZoomControls(); flowZoomValue.blur(); }
  });
  flowZoomValue.addEventListener('blur', applyZoomValueInput);
