...
(function boot() {
  ...
  function drawNode(ctx, c, n) {
    ...
    if (flowFlame) {
      ...
      var isMatch = searching && flowMatchSelf[flowIdxBySeq[n.seq]];
      var fill;
      if (isMatch) {
    ...
    // Flame frames carry only a name, the way a classic flame graph reads: width already
    // says how long the call took, and the exact number, the share, the repeat count are
    // all one hover away in the tooltip. Printing them on every box too is the clutter the
  ...
  /**
   * A multi-point path with each interior corner rounded via arcTo, instead of the sharp
   * right angles a series of lineTo calls would give — square corners on several closely
   * stacked connectors read as a jumble of jagged segments; rounded ones read as one
   * deliberate path, the way a drawn flowchart or a real diagramming tool would render it.
   */
  function elbowPath(ctx, pts) {
    ctx.beginPath();
    ctx.moveTo(pts[0].x, pts[0].y);
    for (var i = 1; i < pts.length - 1; i++) {
      ctx.arcTo(pts[i].x, pts[i].y, pts[i + 1].x, pts[i + 1].y, CHART_ELBOW_R);
    }
  ...
  /**
   * The elbow's corner points, shared by drawChartEdge and the Play marker so the dot
   * rides the exact line drawn on screen instead of cutting a straight shortcut through
   * whatever boxes happen to sit between two steps.
   */
  function chartEdgePoints(e) {
    if (e.rail && chartVertical) {
      // Out to a rail past the widest row, down to End's band, then left into End's side.
      return [
        { x: e.x1, y: e.y1 }, { x: e.railX, y: e.y1 },
        { x: e.railX, y: e.y2 }, { x: e.x2, y: e.y2 }
      ];
    }
    if (e.rail) {
      // Down past the last row to a rail, across, then up into End's underside.
      return [
        { x: e.x1, y: e.y1 }, { x: e.x1, y: e.railY },
        { x: e.x2, y: e.railY }, { x: e.x2, y: e.y2 }
      ];
    }
    if (chartVertical) {
      var midY = e.bend != null ? e.bend : (e.y1 + e.y2) / 2;
      return [
        { x: e.x1, y: e.y1 }, { x: e.x1, y: midY },
        { x: e.x2, y: midY }, { x: e.x2, y: e.y2 }
      ];
    }
    var midX = e.bend != null ? e.bend : (e.x1 + e.x2) / 2;
    return [
      { x: e.x1, y: e.y1 }, { x: midX, y: e.y1 },
      { x: midX, y: e.y2 }, { x: e.x2, y: e.y2 }
    ];
  }

  function chartEdgeArrowDir(e) {
    if (e.rail && chartVertical) return { dx: -1, dy: 0 };
    if (e.rail) return { dx: 0, dy: -1 };
    if (chartVertical) return { dx: 0, dy: 1 };
    return { dx: 1, dy: 0 };
  }

  function drawChartEdge(ctx, c, e) {
    elbowPath(ctx, chartEdgePoints(e));
    var dir = chartEdgeArrowDir(e);
  ...
  function drawChartNode(ctx, c, n) {
    ...
    if (n.total > 0 && flowRootTotal > 0) {
      var pct = (n.total / flowRootTotal) * 100;
      var pctTxt = (pct >= 10 ? Math.round(pct) : pct >= 1 ? pct.toFixed(1)
        : pct >= 0.1 ? '<1' : '<0.1') + '%';
  ...
  function syncFlowSubViewControls() {
    ...
    if (flowMinWSel) flowMinWSel.hidden = !flowFlame;
  }
